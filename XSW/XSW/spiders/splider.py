...
from XSW.items import XswItem

class Mysplider(scrapy.Spider):

    name = 'XSW'
    alolowed_domaine = ['http://www.x23us.com']
    ...
    def get_chapterurl(self, response):
        ...
        categroy = BeautifulSoup(response.text, 'lxml').find('table').find('a').get_text
        ...
        item['categroy'] = str(categroy).replace('/', '')
